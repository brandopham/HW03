...
def login_required(f):
    ...
    def wrap(*args, **kwargs):
        ...
        if 'logged_in' in session:
            ...
            flash('You will need to log in first.')
...
def ser():
    g.db = connect_db()
    cur = g.db.execute("select * from students where name = ? ",
                       (request.form['search'],))
    row = cur.fetchall()
    return render_template("index.html", row=row)
...
def delete():
    g.db = connect_db()
    g.db.execute("delete from students where name = ? ",
                 (request.form['delete'],))
    g.db.commit()
...
def add():
    g.db = connect_db()
    g.db.execute('INSERT INTO students (name,mark1,mark2,total,grade) VALUES(?,?,?,?,?)', [
                 request.form['name'], request.form['mark1'], request.form['mark2'], request.form['total'], request.form['grade']])
    g.db.commit()
    flash('posted')
